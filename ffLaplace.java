...
public class ffLaplace {
    public static BufferedImage image_laplace(BufferedImage srcImage,int x) {
		...
		for (int j = 0; j < h; j++) {
			for(int i=0; i<w; i++) {	
				...
				{   
					...
					r1=(-1)*(rs[i+1][j]+rs[i-1][j]+rs[i][j+1]+rs[i][j-1])+4*rs[i][j];
					g1=(-1)*(gs[i+1][j]+gs[i-1][j]+gs[i][j+1]+gs[i][j-1])+4*gs[i][j];
					b1=(-1)*(bs[i+1][j]+bs[i-1][j]+bs[i][j+1]+bs[i][j-1])+4*bs[i][j];
					if(a==1){
					  if(r1>=d&&g1>=d&&b1>=d){	
					    rgb[0]=255;
					    rgb[1]=255;
