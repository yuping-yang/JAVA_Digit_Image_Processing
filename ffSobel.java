...
public class ffSobel {
    public static BufferedImage image_sobel(BufferedImage srcImage,int x) {
		...
		for (int j = 0; j < h; j++) {
			for(int i=0; i<w; i++) {	
				...
				{   
					...
					sx=(rs[i+1][j-1]+2*rs[i+1][j]+rs[i+1][j+1])-(rs[i-1][j-1]+2*rs[i-1][j]+rs[i-1][j+1]);
					sy=(rs[i-1][j-1]+2*rs[i][j-1]+rs[i+1][j-1])-(rs[i-1][j+1]+2*rs[i][j+1]+rs[i+1][j+1]);
					r1=(int)Math.sqrt(sx*sx+sy*sy);
					sx=(rs[i+1][j-1]+2*rs[i+1][j]+rs[i+1][j+1])-(rs[i-1][j-1]+2*rs[i-1][j]+rs[i-1][j+1]);
					sy=(rs[i-1][j-1]+2*rs[i][j-1]+rs[i+1][j-1])-(rs[i-1][j+1]+2*rs[i][j+1]+rs[i+1][j+1]);
					g1=(int)Math.sqrt(sx*sx+sy*sy);
					sx=(rs[i+1][j-1]+2*rs[i+1][j]+rs[i+1][j+1])-(rs[i-1][j-1]+2*rs[i-1][j]+rs[i-1][j+1]);
					sy=(rs[i-1][j-1]+2*rs[i][j-1]+rs[i+1][j-1])-(rs[i-1][j+1]+2*rs[i][j+1]+rs[i+1][j+1]);
					b1=(int)Math.sqrt(sx*sx+sy*sy);
