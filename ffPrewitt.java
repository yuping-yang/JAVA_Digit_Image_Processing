...
public class ffPrewitt {
    public static BufferedImage image_prewitt(BufferedImage srcImage,int x) {
		...
		for (int j = 0; j < h; j++) {
			for(int i=0; i<w; i++) {	
				...
				{   
					...
					sx=-(rs[i+1][j-1]+rs[i+1][j]+rs[i+1][j+1])+(rs[i-1][j-1]+rs[i-1][j]+rs[i-1][j+1]);
					sy=(rs[i-1][j-1]+rs[i][j-1]+rs[i+1][j-1])-(rs[i-1][j+1]+rs[i][j+1]+rs[i+1][j+1]);
